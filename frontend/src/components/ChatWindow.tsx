              ...
              {quizQuestions.length > 0 ? (
                ...
                <div className="h-full flex flex-col items-center justify-center text-center space-y-8">
                  ...
                  <div className="space-y-4">
                    <h3 className="text-2xl font-bold text-slate-800 tracking-tight">Crafting Your Session</h3>
                    <p className="text-[10px] text-slate-400 font-bold uppercase tracking-[0.25em]">Personalizing Intelligence...</p>
                  </div>
                </div>
              )}
            </motion.div>
              ...
              <div className="space-y-8">
                ...
                <div className="space-y-4">
                  <h2 className="text-5xl font-bold tracking-tight text-slate-800">
                    Hey, <span className="text-black">Scholar!</span>
                  </h2>
            ...
            >
              ...
              {error && (
                <motion.div 
                  ...
                  className="bg-red-50/80 backdrop-blur-md border border-red-100 text-red-600 text-[13px] px-8 py-6 rounded-[2.2rem] mb-12 flex items-center gap-6 shadow-xl"
                >
                  <div className="h-2.5 w-2.5 rounded-full bg-red-400 animate-pulse" />
                  <span className="font-bold flex-1 uppercase tracking-wider">{error}</span>
                  <button 
                    onClick={() => {
                        const lastUserMsg = [...messages].reverse().find(m => m.role === 'user');
                        if (lastUserMsg) onSendMessage(lastUserMsg.content);
                    }}
                    className="px-6 py-2.5 bg-white rounded-2xl text-[11px] font-bold hover:bg-red-600 hover:text-white transition-all shadow-md active:scale-95"
                  >
                    RETRY
                  </button>
                </motion.div>
      ...
      <div className={cn(
        ...
        <div className="max-w-4xl mx-auto relative group">
          ...
          <form 
            ...
            className="relative glass-card-vibrant p-3 rounded-[3rem] flex items-center gap-4 border-white shadow-2xl transition-all group-focus-within:border-vibrant-blue/30 group-focus-within:scale-[1.01]"
          >
            <input 
              type="file" 
              ref={fileInputRef} 
            ...
            <input
              ...
              disabled={loading}
              className="flex-1 h-16 bg-transparent border-none focus:ring-0 text-slate-800 font-semibold placeholder:text-slate-400 px-5 text-[16px]"
            />
            <button 
              type="submit" 
              disabled={!input.trim() || loading}
              className="h-16 px-10 rounded-[2.4rem] premium-gradient text-white font-bold text-[14px] shadow-2xl shadow-vibrant-blue/25 hover:shadow-vibrant-blue/40 transition-all hover:scale-[1.02] active:scale-[0.95] flex items-center gap-4 disabled:grayscale disabled:opacity-50 uppercase tracking-widest"
            >
              Send
